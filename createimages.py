            ...
            for m2 in range(width):
                for n2 in range(factor * m2, factor * (m2 + 1) - 1):
                    for n1 in range(factor * m1, factor * (m1 + 1) - 1):
                        sum = sum + img[n1 + x, n2 + y] * (1 - x1) * (1 - y1) + img[n1 + x + 1, n2 + y + 1] * x1 * y1 + \
                              img[n1 + x, n2 + y + 1] * (1 - x1) * y1 + img[n1 + x + 1, n2 + y] * x1 * (1 - y1)
...
image = Image.open('/Users/chosenone/Desktop/img1_1.tif')
create_images(image, 4, 4)
